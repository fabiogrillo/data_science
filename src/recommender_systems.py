...
user_interest_vectors = [
    make_user_interest_vector(user_interests) for user_interests in users_interests
]
...
def user_based_suggestions(user_id: int, include_current_interests: bool = False):
    ...
    suggestions = sorted(
        suggestions.items(), key=lambda pair: pair[-1], reverse=True  # weight
    )
...
def main():
    ...
    avg_ratings = [
        (sum(title_ratings) / len(title_ratings), movie_id)
    ...
    baseline_error = sum((rating.rating - avg_rating) ** 2 for rating in test) / len(
        test
    )
