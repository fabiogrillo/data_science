...
import operator, math, random, json, tqdm, mnist
from neural_networks import sigmoid, binary_encode, fizz_buzz_encode, argmax
from probability import inverse_normal_cdf
from linear_algebra import dot, squared_distance
...
class Sigmoid(Layer):
    ...
    def backward(self, gradient):
        return tensor_combine(
            lambda sig, grad: sig * (1 - sig) * grad, self.sigmoids, gradient
        )
    ...
    def step(self, layer):
        for param, grad in zip(layer.params(), layer.grads()):
            ...
            param[:] = tensor_combine(
                lambda param, grad: param - grad * self.lr, param, grad
            )
...
class Relu(Layer):
    ...
    def backward(self, gradient):
        return tensor_combine(
            lambda x, grad: grad if x > 0 else 0, self.input, gradient
        )
...
def softmax(tensor):
    ...
    if is_1d(tensor):
        ...
        return [
            exp_i / sum_of_exps for exp_i in exps  # Probability is the fraction
        ]  # of the total weight.
    else:
...
def load_weights(model, filename):
    ...
    assert all(
        shape(param) == shape(weight) for param, weight in zip(model.params(), weights)
    )
...
def main():
    ...
    train_images = [
        [(pixel - avg) / 256 for row in image for pixel in row]
        for image in train_images
    ]
    test_images = [
        [(pixel - avg) / 256 for row in image for pixel in row] for image in test_images
    ]
