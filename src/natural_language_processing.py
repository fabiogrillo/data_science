...
assert cosine_similarity([1.0, 1, 1], [2.0, 2, 2]) == 1, "same direction"
assert cosine_similarity([-1.0, -1], [2.0, 2]) == -1, "opposite direction"
assert cosine_similarity([1.0, 0], [0.0, 1]) == 0, "orthogonal"
...
class SimpleRnn(Layer):
    ...
    def backward(self, gradient: Tensor):
        ...
        a_grad = [
            gradient[h] * (1 - self.hidden[h] ** 2) for h in range(self.hidden_dim)
        ]
...
def main():
    ...
    vocab = Vocabulary(
        word for sentence_words in tokenized_sentences for word in sentence_words
    )
    ...
    pairs = [
        (cosine_similarity(embedding[w1], embedding[w2]), w1, w2)
        for w1 in vocab.w2i
        for w2 in vocab.w2i
        if w1 < w2
    ]
