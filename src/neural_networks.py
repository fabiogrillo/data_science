from linear_algebra import dot, Vector, squared_distance
...
def perceptron_output(weights, bias, x):
    """Returns 1 if the perceptron 'fiires', 0 if not"""
...
xor_network = [  # hidden layer
    [[20.0, 20, -30], [20.0, 20, -10]],  # 'and' neuron  # 'or'  neuron
    # output layer
    [[-60.0, 60, -30]],
...
def sqerror_gradients(network, input_vector, target_vector):
    ...
    make a prediciton and compute the gradient of the squared error loss
    with respect to the neuron weights
    """
    # forward pass
    hidden_outputs, outputs = feed_forward(network, input_vector)

    # gradients with respect to ouput neuron pre-activation ouptuts
    output_deltas = [
        output * (1 - output) * (output - target)
        for output, target in zip(outputs, target_vector)
    ]

    # gradients with respect to ouput neuron weights
    output_grads = [
        [output_deltas[i] * hidden_output for hidden_output in hidden_outputs + [1]]
        for i, output_neuron in enumerate(network[1])
    ]

    # gradients with respect to hidden neuron pre-activation outpus
    hidden_deltas = [
        hidden_output
        * (1 - hidden_output)
...
[  # hidden layer
    [[7, 7, -3], [5, 5, -8]],  # computes OR  # computes AND
    # output layer
...
def main():
    ...
    network = [  # hidden layer: 2 inputs -> 2 outputs
        ...
        # outputs layer: 2 inputs -> 1 outpus
    ...
    for epoch in tqdm.trange(20000, desc="neural net for xor"):
        for x, y in zip(xs, ys):
            ...
            network = [
                [
                    gradient_step(neuron, grad, -learnin_rate)
    ...
    with tqdm.trange(500) as t:
        for epoch in t:
            ...
            for x, y in zip(xs, ys):
                ...
                # Take a gradient stpe for eahc neuron in each layer
                network = [
                    [
                        gradient_step(neuron, grad, -learnin_rate)
                        for neuron, grad in zip(layer, layer_grad)
